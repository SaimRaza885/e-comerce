...
const OrderCard = ({ order }) => {
  ...
  const handleStatusChange = async (e) => {
    ...
    try {
      ...
      // Optional: show toast notification for success
    } catch (error) {
  ...
  return (
    <div className="bg-white shadow-lg rounded-xl p-6 hover:shadow-2xl transition-shadow duration-300 border border-gray-100">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-4 text-sm text-gray-600">
        <p>
          <span className="font-medium">Total:</span> {order.totalPrice} PKR
        </p>
        <p>
          <span className="font-medium">Customer:</span> {order.user?.fullName} ({order.user?.email})
        </p>
        <p>
          <span className="font-medium">Address:</span> {order.street}, {order.city}, {order.country}
        </p>
        <p>
